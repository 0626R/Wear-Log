...
>
  ...
  @endif
    ...
    <div class="col-md-8">
        <div class="mb-3">
            <label class="form-label d-block">カテゴリ</label>
        
            @php
            // バリデーションエラー時に old() を優先して選択状態を復元
            $checked = old('categories', $selectedCategories ?? []);
            @endphp
            ...
            @foreach($categories as $cat)
                ...
                <label class="form-check-label">
                    ...
                    name="categories[]"
                    value="{{ $cat->id }}"
                    @checked(in_array($cat->id, $checked, true))
                    >
                    {{ $cat->name }}
                </label>
                </div>
